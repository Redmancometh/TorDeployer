 ...
 * Modified by LevelUp:
 *
 * - Added baseName to support deserialization of wrapped types.
 */
public final class RuntimeTypeAdapterFactory<T> implements TypeAdapterFactory
{
    ...
    private RuntimeTypeAdapterFactory(String baseName, Class<?> baseType, String typeFieldName)
    {
    ...
    /**
     * Creates a new runtime type adapter for {@code baseType} wrapped with {@code baseName} using
     * {@code typeFieldName} as the type field name. Type field names are case sensitive.
     */
    public static <T> RuntimeTypeAdapterFactory<T> of(String baseName, Class<T> baseType, String typeFieldName)
    {
        return new RuntimeTypeAdapterFactory<>(baseName, baseType, typeFieldName);
    }

    /**
     * Creates a new runtime type adapter for {@code baseType} wrapped with {@code baseName} using
     * {@code "type"} as the type field name. Type field names are case sensitive.
     */
    public static <T> RuntimeTypeAdapterFactory<T> of(String baseName, Class<T> baseType)
    {
        return of(baseName, baseType, "type");
    }
    ...
    {
        return of(null, baseType, typeFieldName);
    }
    ...
    public <R> TypeAdapter<R> create(Gson gson, TypeToken<R> type)
    {
        ...
        return new TypeAdapter<R>()
        {
            ...
            public R read(JsonReader in) throws IOException
            {
                ...
                if (baseName != null)
                {
                    JsonElement baseJsonElement = jsonObject.get(baseName);
                    if (baseJsonElement == null)
                    {
                        throw new JsonParseException("cannot deserialize " + baseType + " because it is not wrapped with " + baseName);
                    }
                    jsonObject = jsonObject.get(baseName).getAsJsonObject();
                }
